...
def add_details(tokenizer, model, device="cuda"):
    mm_use_im_start_end = getattr(model.config, "mm_use_im_start_end", False)
    mm_use_im_patch_token = getattr(model.config, "mm_use_im_patch_token", True)
    if mm_use_im_patch_token:
        tokenizer.add_tokens([DEFAULT_IMAGE_PATCH_TOKEN], special_tokens=True)
...
def build_model(model_path=None, model_base=None, model_name=None, load_8bit=None, load_4bit=None):
    # Model
    disable_torch_init()
...
def llava_infer(image, ocr_tokens, tokenizer, model, image_processor, model_name, context_len=2048):
    ...
    argconv_mode=None
    if argconv_mode is not None and argconv_mode != conv_mode:
    ...
    if ocr_tokens == "":
        inp = "Please provide a caption for the provided image."
    else:
        inp = "Please use words in " + ocr_tokens + " to describe the image."
    # only first message
