...
from pythia.utils.llama2_model_utils import load_model, load_peft_model, load_llama_from_config
...
def build_llama2(
    ...
    top_p: float=0.7, # [optional] If set to float < 1, only the smallest set of most probable tokens with probabilities that add up to top_p or higher are kept for generation.
    temperature: float=0.05, # [optional] The value used to modulate the next token probabilities.
    top_k: int=16, # [optional] The number of highest probability vocabulary tokens to keep for top-k-filtering.
    repetition_penalty: float=1.0, #The parameter for repetition penalty. 1.0 means no penalty.
    length_penalty: int=4, #[optional] Exponential penalty to the length that is used with beam-based generation. 
...
def generate(
    ...
    **kwargs
    ...
    if index_result == -1:
        ...
        index_result += 1
        return output_llm[index_result + len("Answer:"):]
    # return result
...
def main(
    ...
    prompt_file: str='prompt_file.txt',
    seed: int=42, #seed value for reproducibility
    do_sample: bool=True, #Whether or not to use sampling ; use greedy decoding otherwise.
    min_length: int=None, #The minimum length of the sequence to be generated, input prompt + min_new_tokens
    use_cache: bool=True,  #[optional] Whether or not the model should use the past last key/values attentions Whether or not the model should use the past last key/values attentions (if applicable to the model) to speed up decoding.
    ...
    top_k: int=64, # [optional] The number of highest probability vocabulary tokens to keep for top-k-filtering.
    repetition_penalty: float=1.0, #The parameter for repetition penalty. 1.0 means no penalty.
    length_penalty: int=4, #[optional] Exponential penalty to the length that is used with beam-based generation. 
    enable_azure_content_safety: bool=False, # Enable safety check with Azure content safety api
    enable_sensitive_topics: bool=False, # Enable check for sensitive topics using AuditNLG APIs
