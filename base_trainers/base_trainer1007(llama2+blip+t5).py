    ...
    def evaluate(self, loader, use_tqdm=False, single_batch=False):
        ...
        with torch.no_grad():
            ...
            if self.config.training_parameters.qa_gen:
                ...
                caption_model = BlipForConditionalGeneration.from_pretrained(self.config.training_parameters.blip_caption_large_path, torch_dtype=torch.float32)
                self.caption_processor = AutoProcessor.from_pretrained(self.config.training_parameters.blip_caption_large_path)
                caption_model.eval()
                self.caption_model = caption_model
                print("Load T5 for QA generation")
                self.question_generation_tokenizer = T5Tokenizer.from_pretrained(
                    self.config.training_parameters.t5_large_lm_adapt_path
                )
                self.question_generation_model = T5ForConditionalGeneration.from_pretrained(
                    self.config.training_parameters.t5_large_lm_adapt_path
                )
                cache_file = self.config.training_parameters.T5_large_QG_path
                checkpoint = torch.load(cache_file, map_location="cpu")
                state_dict = checkpoint["model"]
                self.question_generation_model.load_state_dict(state_dict)
